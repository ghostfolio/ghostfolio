...
| `DIRECT_URL`                | `string` (optional)   |                       | The direct database connection URL used by the Prisma CLI (e.g. for schema migrations) and seeding, bypassing any connection poolers. Falls back to `DATABASE_URL` if not provided.    |
