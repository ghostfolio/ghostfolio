 ...
 */
export function runVerificationChecks(ctx: VerificationContext): {
  responseText: string;
...
function checkDataBackedClaims(
  ...
  const normalize = (s: string) => s.replace(/[$%,]/g, '').replace(/^0+/, '');
...
function checkPortfolioScope(
  ...
  const contextualOutOfScope = outOfScope.filter((ticker) => {
    ...
    const surrounding = responseText
      .substring(Math.max(0, idx - 80), Math.min(responseText.length, idx + 80))
      .toLowerCase();
    return (
